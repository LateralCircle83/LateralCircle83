...
ll.registerPlugin("locatepl", "查找附近自然建筑", [2, 4, 3], { "开源许可证": "GPL-2.0" })


log("locatepl插件以加载，作者lateralCircle83")
//log("将要启动locatepl_editor")
//mc.runcmd("ll load ./plugins/locatepl/locatepl_editor.js")
//定义配置文件
...
//主线部分
// 是否拥有api
var direction_api_add, direction_api_list, NavigationAPI_api_new, NavigationAPI_api_has
if (!ll.hasExported("direction", "add_direction")) {
    logger.error("未载入罗盘api,如有需要请前往'https://www.minebbs.com/resources/api.5136/'获取")
    direction_api_add, direction_api_list = null
    // 赋予生命
} else { direction_api_add = ll.import('direction', 'add_direction')/*; direction_api_del = ll.import('direction', 'del_direction')*/; direction_api_list = ll.import('direction', 'list_direction') }
if (!ll.hasExported("NavAPI_newTask")) {
    logger.error("未载入导航api,如有需要请前往'https://www.minebbs.com/resources/navigationapi-api.4423/'获取")
    NavigationAPI_api_new, NavigationAPI_api_has = null
    // 赋予生命
} else { NavigationAPI_api_new = ll.import("NavAPI_newTask"); NavigationAPI_api_has = ll.import('NavAPI_hasTask') }
// 定义命令
mc.listen("onServerStarted", () => {
    var command = mc.newCommand("locatepl", "查找自然建筑", PermType.Any, 0x80, "lcpl")
    command.setEnum("with_gui_enum", ["gui"])
    // 必选参数1.gui
    command.optional("with_gui", ParamType.Enum, "with_gui_enum")
...
function idonknow() {
    ...
    firstfm.addButton("§clocatepl_editor")
...
function menu_indx(pl) {
    pl.sendForm(idonknow(), function (pl, id) {
        ...
        if (id == 2) (pl.runcmd("/lcpl_e"))
        if (id == 3) { locateplknowo(pl) }//点击一直对象
...
function locatepl(pl) {
    ...
    let dir_api = readconf.get("use_direction_api")
    let nav_api = readconf.get("use_NavigationAPI_api")
    ...
    pl.sendForm(form_exectend(plworld), (pl, id) => {
        if (id != null) {
            ...
            // 二级菜单：付费标准
            let got_money = mc.newCustomForm()
            got_money.setTitle(str_all.cn_name)
            got_money.addLabel("您当前拥有的小钱钱：" + String(pl_money) + "\n请选择您要的套餐")
            got_money.addSwitch("启用罗盘标记", true)
            got_money.addSwitch("启用导航", true)
            got_money.addStepSlider("查找方式", ["普通查找，可能找到被别人搞过的：" + String(locate_money_common), "在新区块查找，没被别人搞过哦~：" + String(locate_money_useNewChunksOnly)])
            pl.sendForm(got_money,/*pl.sendSimpleForm(str_all.cn_name, "您当前拥有的小钱钱：" + String(pl_money) + "\n请选择您要的套餐", ["普通查找，可能找到被别人搞过的：" + String(locate_money_common), "在新区块查找，没被别人搞过哦~：" + String(locate_money_useNewChunksOnly)], ["", ""], */function (pl, arr) {
                ...
                if (commond.output == `Syntax error: Unexpected "${enname}": at "structure >>${enname}<< ${bool[idd]}"`) {
                    pl.tell("找不到此类建筑")
                    return
                } else {
                    // 运算输出区
                ...
                switch (is_llmoney) {
                    case true:
                        ...
                        break
                    case false:
                        let ob = mc.getScoreObjective(maney_way)||mc.newScoreObjective(maney_way,maney_way)
                ...
                if (arr[2]) {
                    if (nav_api) {
                        if (NavigationAPI_api_new) {
                            if (NavigationAPI_api_has(pl.xuid)) {
                                pl.tell("[locatepl]请先完成目前的导航")
                            } else {
                                NavigationAPI_api_new(pl.xuid, { "pos": { "x": Number(thex), "y": 64, "z": Number(thez), "dimId": testword }, "name": str_all.cn_name })
                            }
                        } else { pl.tell("[locatepl]快点告诉你腐竹没装罗盘插件（给作者lgc2333打钱（bushi）") }
                    } else { pl.tell("[locatepl]你们腐竹竟然没开启导航功能") }
                }
            })
...
function oplesthh() {
    ...
    fmop.addSwitch("是否启用罗盘api", Boolean(dir_api))
    fmop.addSwitch("是否启用导航api", Boolean(nav_api))
    ...
    if (pl.isOP() == true) {
        pl.sendForm(oplesthh(), (_pla, callbacket) => {
            if (callbacket != null) {
                ...
                readconf.set("use_direction_api", Boolean(callbacket[5]))
                readconf.set("use_NavigationAPI_api", Boolean(callbacket[6]))
                readconf.set("money", callbacket[7])
            }
        })
