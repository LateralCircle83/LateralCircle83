...
}else{checkUpdate("direction",5136,[1,2])}
var all_ids = {}
...
// 抛出命令
let cmd = mc.newCommand("endir", "停止所有指向", PermType.Any)
cmd.overload()
cmd.setCallback((_cmd, origin, output, _res) => {
    switch (origin.type) {
        case 0:
            //还没定义
 ...
 * @returns {Boolean}
 */
function add_direction(pl, pos, name) {
    ...
    var sert = setInterval(function () {
        ...
        for (var e = -13; e != 14; e++) {
            ...
            switch (xiangdui + e) {
                ...
                default:
                    // 替换api指向
                    if (loca_dir.indexOf(xiangdui + e) != -1) {
                        obj[e.toString()] = "§1-§r"
                    } else {
                        //没有替换为-
                        obj[e.toString()] = "-"
...
function with_pos_dir(pl) {
    ...
    for (var i = 0, len = Object.keys(date).length; i < len; i++) {
        let date_this = Object.values(date)[i]
