...
if (!ll.hasExported(`cup`, `checkUpdate`)) {
    ...
    checkUpdate("direction",5136,[1,5,1])}
var all_ids = {}
...
cmd.setCallback((_cmd, origin, output, res) => {
    switch (origin.type) {
        case 0:
            ...
            switch(res.act){
                case "start":
                    if(pl.hasTag("on_dir_open_true")) {output.error("你已经开启罗盘！");return}
                    pl.addTag("on_dir_open_true")
                    start(pl)
                    break
                case "stop":
                    if(!pl.hasTag("on_dir_open_true")){output.error("您没开启罗盘！");return}
...
function start(_pl){
    ...
    var sert = setInterval(function () {
        ...
        for (var e = -13; e != 14; e++) {
            ...
            switch (xiangdui + e) {
                case 0:
                    obj[e.toString()] = ((kys==-1)&&"§c南§r")||"§a南§r"
                    break
                case 27:
                    obj[e.toString()] = ((kys==-1)&&"§c西§r")||"§a西§r"
                    break
                case 54:
                    obj[e.toString()] = ((kys==-1)&&"§c北§r")||"§a北§r"
                    break
                case 81:
                    obj[e.toString()] = ((kys==-1)&&"§c东§r")||"§a东§r"
                    break
                case 108:
                    obj[e.toString()] = ((kys==-1)&&"§c南§r")||"§a南§r"
                    break
...
mc.listen("onLeft",(pl)=>{
    if(!pl.hasTag("on_dir_open_true")){return}
    stoper(pl)
    return
})
// 获取玩家定位项全部信息
